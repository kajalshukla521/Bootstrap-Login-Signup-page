document.addEventListener("DOMContentLoaded", () => {
  if (document.getElementById("loginForm")) {
    ...
    sendOtpButton.addEventListener("click", () => {
      ...
      alert(
        `OTP has been sent to ${emailPhoneValue}. (For testing: ${generatedOtp})`
      );
...
document
  ...
  .addEventListener("submit", function (event) {
    ...
    //  the process of sending a call or SMS
    if (emailPhone) {
      responseMessage.innerHTML = `<p class="text-success">A call or SMS has been sent to ${emailPhone}.</p>`;
    } else {
      responseMessage.innerHTML =
        '<p class="text-danger">Please enter a valid email or phone number.</p>';
    }
  });
